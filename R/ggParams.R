...
ggThemeParamsUI = function(id) {
  ...
  tagList(selectInput(ns("legendPosition"), "Legend position:",
          ...
          sliderInput(ns("largeSizeLarge"), label = "Font size large:", min = 9, max = 36, value = 24, step = 1),
...
ggMakeThemeParams = function(input, output, session)
{
  ...
  observe({
    ...
    {
      themes$axis.text.x = theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1))

    } else {
      themes$axis.text.x = theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))
    }
  })
